...
P1 Collections requests `accounting.reports.aged.read` and `accounting.contacts.read` in addition to the statement scopes. A refresh token keeps only the scopes from the last consent, so an existing connection must disconnect and connect again. Do not add invoice, bank-transaction, or budget scopes in this pull. This table is the map for later phases.
