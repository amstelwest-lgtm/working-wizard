  ...
  return (
    <div className="fixed bottom-20 right-4 z-50 flex flex-col items-end gap-2">
      {surface?.clientId && notes.length > 0 && (
        <button
          type="button"
          onClick={() => openArchive(resolvedCount > 0 && count === 0 ? "resolved" : "open")}
          title={
            resolvedCount > 0
              ? `Notes archive (${resolvedCount} resolved)`
              : "Notes archive"
          }
          aria-label="Open notes archive"
          className="flex h-10 w-10 items-center justify-center rounded-full border border-[#d4a550]/25 bg-[#d4a550]/12 text-[#d4a550]/70 backdrop-blur-md transition-all duration-200 hover:border-[#d4a550]/60 hover:bg-[#d4a550]/25 hover:text-[#d4a550]"
        >
      ...
      <button
        ...
        className={`relative flex h-10 w-10 items-center justify-center rounded-full border backdrop-blur-md transition-all duration-200 ${
          !surface?.clientId
