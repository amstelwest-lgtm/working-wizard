        ...
        {steps.map((s, i) => {
          ...
          return (
            <Fragment key={i}>
              ...
              >
                {s.kind === "decrease"
                  ? `(${fmtRandCompact(Math.abs(s.delta), market)})`
                  : fmtRandCompact(s.delta, market)}
              </Text>
...
function MarginLadder({ d, accent }: { d: ProfitabilityData; accent: string }) {
  ...
  const rows = [
    ...
    },
    {
      ...
      color: d.net_profit >= 0 ? C.green : C.red,
    },
  ];
  return (
    <View>
      {rows.map((r) => (
        <View key={r.name} style={ladderStyles.row}>
          ...
          <Text style={ladderStyles.amount}>{fmtRand(r.amount, market)}</Text>
...
function ComparisonTable({ rows, accent }: { rows: CompRow[]; accent: string }) {
  const market = usePdfMarket();
  const unit = currencySymbol(market);
  const headers = [
    { label: "Metric", flex: 2 },
    { label: `Current (${unit})`, flex: 1.5, align: "right" as const },
    { label: "%", flex: 0.7, align: "right" as const },
    { label: `Prior (${unit})`, flex: 1.5, align: "right" as const },
    { label: "%", flex: 0.7, align: "right" as const },
        ...
        return (
          ...
          >
            ...
            <Text style={[tblStyles.cell, { flex: 1.5, textAlign: "right" }]}>
              {fmtRand(row.curVal, market)}
            </Text>
            <Text style={[tblStyles.cell, { flex: 0.7, textAlign: "right" }]}>
              {fmtPct(row.curPct)}
            </Text>
            <Text style={[tblStyles.cell, { flex: 1.5, textAlign: "right" }]}>
              {row.priorVal !== undefined ? fmtRand(row.priorVal, market) : "—"}
            </Text>
            <Text style={[tblStyles.cell, { flex: 0.7, textAlign: "right" }]}>
...
export function ProfitabilityWaterfallPDF({
  ...
  const figures: HeadlineFigure[] = [
    {
      label: "Revenue",
      value: fmtRandCompact(d.revenue, m),
