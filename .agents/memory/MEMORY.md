...
- [Advisory OS spine](advisory-state.md) — clients.advisory_state advanced by DB triggers via advisory_apply_event (rules in src/lib/advisory-state.ts, seeded into advisory_transition_rules); recommendations = proposed_next_steps + recommendation_outcomes; resolveNextStep() in src/lib/next-step.ts turns state+facts into one CTA; tests: test:advisory-state, test:recommendations, test:next-step
